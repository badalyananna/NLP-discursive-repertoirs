...
class ModelWithProperties(nn.Module):
    ...
    def forward(self, input_ids, attention_mask):
        ...
        linear1_results = linear1.clone().detach()
        #output1 = nn.functional.relu(linear1)
        #utput2 = nn.functional.relu(linear2)
        #joined = torch.cat((linear1_results, linear2), dim=1)
        output = torch.sigmoid(linear1_results)
        output = self.output(output)
        return linear1, output
