...
df = pd.read_csv(sys.argv[1] + 'mult_train64.csv', na_filter=False)
val_df = pd.read_csv(sys.argv[1] + 'mult_val64.csv', na_filter=False)
test_df = pd.read_csv(sys.argv[1] + 'mult_test64.csv', na_filter=False)
...
train_dataset = HyperionDataset(df, model_name)
val_dataset = HyperionDataset(val_df, model_name)
...
if config['save']:
    model.push_to_hub("multi_label", use_temp_dir=True, use_auth_token=hf_token)
    AutoTokenizer.from_pretrained(model_name).push_to_hub("multi_label", use_temp_dir=True, use_auth_token=hf_token)
