...
class BertClsTrainer():
    ...
    def fit(self, model, train_dataset, val_dataset, batch_size, lr, n_epochs, loss_fn1, loss_fn2):
        ...
        for epoch_i in range(0, n_epochs):
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                with torch.cuda.amp.autocast():
                    output1, logits2 = model(b_input_ids, attention_mask=b_input_mask)
                    loss1 = loss_fn1(output1, b_vectors.to(torch.float32))
                    loss2 = loss_fn2(logits2, b_labels.view(-1))
                    loss = 0.5 * loss1 + loss2

                logits = logits2.detach().cpu()
                label_ids = b_labels.to('cpu')

                batch_metric = self.metric_collection.update(logits.float().softmax(dim=1), label_ids)
    ...
    def test(self, model, test_dataset, batch_size, loss_fn1, loss_fn2):
