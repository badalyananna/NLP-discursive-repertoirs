...
class BertClsTrainer():
    def __init__(self) -> None:
        self.metric_collection = torchmetrics.MetricCollection({
            ...
            'accuracy_micro' : torchmetrics.Accuracy(num_classes=2, average='micro'),
            'accuracy_macro' : torchmetrics.Accuracy(num_classes=2, average='macro'),
            'accuracy_weighted' : torchmetrics.Accuracy(num_classes=2, average='weighted'),
            'accuracy_none' : torchmetrics.Accuracy(num_classes=2, average='none'),

            'f1_micro' : torchmetrics.F1Score(num_classes=2, average='micro'),
            'f1_macro' : torchmetrics.F1Score(num_classes=2, average='macro'),
            'f1_weighted' : torchmetrics.F1Score(num_classes=2, average='weighted'),
            'f1_none' : torchmetrics.F1Score(num_classes=2, average='none'),

            'precision_micro' : torchmetrics.Precision(num_classes=2, average='micro'),
            'precision_macro' : torchmetrics.Precision(num_classes=2, average='macro'),
            'precision_weighted' : torchmetrics.Precision(num_classes=2, average='weighted'),
            'precision_none' : torchmetrics.Precision(num_classes=2, average='none'),

            'recall_micro' : torchmetrics.Recall(num_classes=2, average='micro'),
            'recall_macro' : torchmetrics.Recall(num_classes=2, average='macro'),
            'recall_weighted' : torchmetrics.Recall(num_classes=2, average='weighted'),
            'recall_none' : torchmetrics.Recall(num_classes=2, average='none')
        }) 
        ...
        for epoch_i in range(0, n_epochs):
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                with torch.cuda.amp.autocast():
                    ...
                    logits = output[0]
                    loss = loss_fn(logits, b_vectors.to(torch.float32))

                logits = torch.sigmoid(logits).detach().cpu()
    ...
    def test(self, model, test_dataset, batch_size, loss_fn):
        ...
        for batch in test_dataloader:
            ...
            #batch_pred = torch.nn.functional.sigmoid(logits)
