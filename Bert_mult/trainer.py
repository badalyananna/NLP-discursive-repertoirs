...
class BertClsTrainer():
    def __init__(self) -> None:
        ...
        self.metric_collection = torchmetrics.MetricCollection({
            'accuracy_micro' : torchmetrics.Accuracy(num_classes=num_classes, multiclass=True, average='micro'),
            'accuracy_macro' : torchmetrics.Accuracy(num_classes=num_classes, multiclass=True, average='macro'),
            'accuracy_weighted' : torchmetrics.Accuracy(num_classes=num_classes, multiclass=True, average='weighted'),
            'accuracy_none' : torchmetrics.Accuracy(num_classes=num_classes, multiclass=True, average='none'),

            'f1_micro' : torchmetrics.F1Score(num_classes=num_classes, multiclass=True, average='micro'),
            'f1_macro' : torchmetrics.F1Score(num_classes=num_classes, multiclass=True, average='macro'),
            'f1_weighted' : torchmetrics.F1Score(num_classes=num_classes, multiclass=True, average='weighted'),
            'f1_none' : torchmetrics.F1Score(num_classes=num_classes, multiclass=True, average='none'),

            'precision_micro' : torchmetrics.Precision(num_classes=num_classes, multiclass=True, average='micro'),
            'precision_macro' : torchmetrics.Precision(num_classes=num_classes, multiclass=True, average='macro'),
            'precision_weighted' : torchmetrics.Precision(num_classes=num_classes, multiclass=True, average='weighted'),
            'precision_none' : torchmetrics.Precision(num_classes=num_classes, multiclass=True, average='none'),

            'recall_micro' : torchmetrics.Recall(num_classes=num_classes, multiclass=True, average='micro'),
            'recall_macro' : torchmetrics.Recall(num_classes=num_classes, multiclass=True, average='macro'),
            'recall_weighted' : torchmetrics.Recall(num_classes=num_classes, multiclass=True, average='weighted'),
            'recall_none' : torchmetrics.Recall(num_classes=num_classes, multiclass=True, average='none')
        })  
    ...
    def fit(self, model, train_dataset, val_dataset, batch_size, lr, n_epochs, loss_fn):
        ...
        for epoch_i in range(0, n_epochs):
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                b_vectors = batch['vectors'].to(device)
                ...
                with torch.cuda.amp.autocast():
                    ...
                    logits = output[0]
                    loss = loss_fn(logits, b_vectors)

                logits = logits.softmax(dim=1).detach().cpu()
                vector_ids = b_vectors.to('cpu')

                batch_metric = self.metric_collection.update(logits, vector_ids)
            ...
            for batch in validation_dataloader:
                ...
                batch_metric = self.metric_collection.update(logits.softmax(dim=1), vector_ids)
    ...
    def test(self, model, test_dataset, batch_size, loss_fn):
        ...
        for batch in test_dataloader:
            ...
            batch_metric = self.metric_collection.update(batch_pred, vector_ids)
